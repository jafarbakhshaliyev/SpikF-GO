...
class SFFT(nn.Module):
    """
    Vectorized cuFFT version:
      - rfft_masked(tok, s_bin):  amplitude-aware (spike-masked) FFT
      - rfft_spike_only(s_bin):   strict 0/1 spike signal FFT
    ...
    def rfft_masked(self, tok: torch.Tensor, s_bin: torch.Tensor) -> torch.Tensor:
        ...
        # (tok * s_bin) in time domain → batched rFFT over T*B*E signals of length M
        x = (tok.unsqueeze(0) * s_bin)                      # [T,B,M,E]
    ...
    def irfft_batched(self, Z_t: torch.Tensor) -> torch.Tensor:
        T, B, F, E = Z_t.shape
        x = Z_t.permute(0, 1, 3, 2).contiguous().view(T*B*E, F)         # [T*B*E, F]
...
class EventDrivenDepthwiseComplex(nn.Module):
    ...
    def forward(self, z: torch.Tensor, g_t: torch.Tensor | None = None) -> torch.Tensor:
        ...
        yr = torch.zeros([T, B, self.F, self.D * self.hidden_size_factor],
                              device=z.device)
        yi = torch.zeros([T, B, self.F, self.D * self.hidden_size_factor],
                              device=z.device)

        yr = torch.einsum('tbli,ii->tbli', z.real, self.w1[0]) - \
            torch.einsum('tbli,ii->tbli', z.imag, self.w1[1]) + \
            self.b1[0]
    

        yi = torch.einsum('tbli,ii->tbli', z.imag, self.w1[0]) + \
            torch.einsum('tbli,ii->tbli', z.real, self.w1[1]) + \
            self.b1[1]
...
class SpikingDecoderKeepT(nn.Module):
    ...
    def __init__(self, D: int, L: int, pred_len: int,
        ...
        self.lif1 = MultiStepLIFNode(tau=tau, v_threshold=v_th, detach_reset=True,
                                     surrogate_function=surrogate.ATan(alpha=4.0), backend='torch')
        self.lif2 = MultiStepLIFNode(tau=tau*1.5, v_threshold=v_th*0.8, detach_reset=True,
                                     surrogate_function=surrogate.ATan(alpha=4.0), backend='torch')
        self.lif3 = MultiStepLIFNode(tau=tau*0.7, v_threshold=v_th*1.2, detach_reset=True,
                                     surrogate_function=surrogate.ATan(alpha=4.0), backend='torch')
...
class SpikF_GO(nn.Module):
    """
    Pipeline:
      x [B,L,N] -> flatten M=N*L -> embed -> spike-encode over T -> S-FFT (event-driven)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        x: [B, L, N]
        -> preds: [B, P, N]
           aux:   dict of monitors (gate L0, gating fractions)
        """
        B, L, N = x.shape
        ...
        y_time_t = self.sfft.irfft_batched(Z_t) 
        y_t = y_time_t.view(T, B, N, L, E).permute(0, 1, 2, 4, 3).contiguous()  # [T,B,N,E,L]

        # 8) T-preserving decoder + learned T readout
        preds, dec_stats = self.decoder(y_t)         # [B,P,N]
        preds = preds * std + mean

        aux = {
            'enc_rate': enc_rate_cont,                  # <-- use this for loss
