...
class SpikingFourierBlock(nn.Module):
    ...
    def __init__(
        ...
        # NEW: gates after affines
        self.ga1 = ComplexLIFGate(tau=tau, v_th=v_th)  # after a1 (E)
        self.ga2 = ComplexLIFGate(tau=tau, v_th=v_th)  # after a2 (H)
        self.ga3 = ComplexLIFGate(tau=tau, v_th=v_th)  # after a3 (E)
    ...
    def forward(self, Z: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        with torch.no_grad():
            ...
            # report both affine-gates and linear-gates
            stats["gate_aff_frac_1"] = GA1.mean().detach()
            stats["gate_aff_frac_2"] = GA2.mean().detach()
            stats["gate_aff_frac_3"] = GA3.mean().detach()
